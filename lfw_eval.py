...
# from megaface_utils import get_image as read_image_from_file

angles_file = os.path.join(LFW_DIR, 'angles.txt')
lfw_pickle = os.path.join(LFW_DIR, 'lfw_funneled.pkl')


extracted_folder = os.path.join(LFW_DIR, 'lfw_funneled')
...
def process():
    subjects = [d for d in os.listdir(extracted_folder) if os.path.isdir(os.path.join(extracted_folder, d))]
    assert (len(subjects) == 5749), "Number of subjects is: {}!".format(len(subjects))

    file_names = []
    for i in tqdm(range(len(subjects))):
        sub = subjects[i]
        folder = os.path.join(extracted_folder, sub)
    ...
    for item in tqdm(file_names):
        ...
        try:
            is_valid, bboxes, landmarks = get_central_face_attributes(filename)
...
def read_image_from_file(filepath, flip = False):   # Return as RGB
...
def evaluate(model):
    ...
    filename = os.path.join(LFW_DIR, 'lfw_test_pair.txt')
...
def visualize(threshold, show=False):
...
def copy_file(old, new):
    old_fn = os.path.join(extracted_folder, old)
...
def lfw_test(model):
    filename = os.path.join(LFW_DIR, 'lfw-funneled.tgz')
