...
##################################### Mobile FaceNet #########################################
# From https://github.com/TreB1eN/InsightFace_Pytorch/blob/master/model.py
...
class Depth_Wise(nn.Module):
     ...
     def forward(self, x):
        ...
        # Rewritten for the JIT compilation
        conved_block = self.conv(x)
        conved_block = self.conv_dw(conved_block)
        conved_block = self.project(conved_block)

        if self.residual:
            output = conved_block + x
        else:
            output = conved_block
